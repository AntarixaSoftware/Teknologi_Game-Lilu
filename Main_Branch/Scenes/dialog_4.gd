...
var dialog_queue: Array = [
	{"name": "Charles", "dialog": "Astaga! Apa ini? Bagaimana bisa ada seperti ini di bawah rumah? Siapa yang taruh benda aneh ini di sini?"},
	{"name": "Charles", "dialog": "Ucok, aku yakin ini ada hubungannya dengan si dukun itu. Jangan-jangan ini bagian dari rencananya."}
]
...
func _ready():
	var start = get_node("/root/Main Scene/Main Character/InteractionArea")
	start.connect("display4", Callable(self, "_on_display4"))
